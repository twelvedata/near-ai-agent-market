...
def test_function_routes_to_correct_rest_endpoint(client, auth_headers):
    ...
    resp = client.post(
        "/invoke",
        json={"input": {"symbol": "AAPL", "function": "TIME_SERIES", "interval": "1day"}},
        headers=auth_headers,
    )
    assert resp.status_code == 200
    assert route.called
    assert "apikey=test-td-key" in str(route.calls.last.request.url)
...
def test_function_key_stripped_from_forwarded_params(client, auth_headers):
    ...
    client.post(
        "/invoke",
        json={"input": {"symbol": "AAPL", "function": "QUOTE"}},
        headers=auth_headers,
    )
...
@respx.mock
def test_rest_failure_without_utool_returns_502(client, auth_headers):
    respx.get(f"{TD_API_BASE}/unknown_func").mock(
        return_value=httpx.Response(404, json={"status": "error"})
    )
    ...
    assert "utool fallback also failed" in resp.json()["detail"]
...
def test_query_goes_to_utool_with_openai_key(client, auth_headers):
    ...
    client.post(
        "/invoke",
        json={"input": {"query": "What is the RSI for AAPL?"}},
        headers=auth_headers,
    )
    assert route.called
...
@respx.mock
def test_query_when_utool_missing_returns_502(client, auth_headers):
    respx.get(f"{TD_MCP_BASE}/utool").mock(
        return_value=httpx.Response(404, text="Not Found")
    )
    resp = client.post(
    ...
    assert resp.status_code == 502
    assert "QUOTE" in resp.json()["detail"]
